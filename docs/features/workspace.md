...
3. **Файлы:** Пользователь открывает File Explorer (`Cmd+B`). 
   - Проводник открывается поверх терминала через [React Portal](knowledge/ui-react-patterns.md).
...
- **Persistent Mounting:** Workspace остается смонтированным всегда после первого открытия. Переход Dashboard ↔ Workspace осуществляется через CSS-скрытие. См. `knowledge/ui-terminal-rendering.md` (раздел 2).
