...
**⚠️ КРИТИЧЕСКОЕ ПРАВИЛО: Chunked Paste + Echo Verification.**
macOS TTYHOG = 1024 bytes — ядро режет любой `term.write()` > 1024 байт. Ink TUI не может собрать фрагментированный bracketed paste.
**Решение:** Использовать `safePasteAndSubmit()` — каждый чанк < 900 байт оборачивается в ПОЛНЫЙ `\x1b[200~` + chunk + `\x1b[201~` (итого < 1024), после каждого чанка ждём echo в PTY output (event-driven). `\r` шлём только после подтверждения последнего чанка. Работает для любого размера текста.
