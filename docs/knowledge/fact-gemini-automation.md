...
### Отслеживание модификаций (mtime)
В отличие от Claude, который всегда создает новый файл, возобновление сессии Gemini (`-r`) модифицирует существующий. Sniper настроен на отслеживание `Math.max(mtimeMs, birthtimeMs)`, что позволяет захватывать ID сессии даже при возобновлении старых диалогов.

---

## 3. Bare Resume Strategy
При вводе команды `gemini -r` (без указания конкретного ID) Gemini CLI автоматически подхватывает последнюю активную сессию проекта.
- **Механизм:** Поскольку в этом случае новый файл не создается и `fs.watch` может не сработать до первого сообщения, система применяет стратегию «Мгновенного поиска».
- **Логика:** Сразу после ввода команды Main-процесс сканирует директорию чатов, находит файл с самым свежим `mtime` и извлекает из него `sessionId`. Это позволяет Timeline появиться мгновенно, не дожидаясь ответа от AI.

---

## 4. Handshake Strategy
