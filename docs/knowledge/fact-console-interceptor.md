...
### Симптомы (без фильтрации)
Консоль разработчика (DevTools) перегружена тысячами сообщений `[Workspace:RENDER]`, что приводит к заметным "фризам" интерфейса при открытой консоли и затрудняет поиск реальных ошибок среди диагностического шума.
