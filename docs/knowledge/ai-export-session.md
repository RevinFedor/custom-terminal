...
### Симптомы
В текстовом файле экспорта (Markdown) разделитель `🔵 FORK` появляется ошибочно после каждого сообщения пользователя, даже если ветвления не было. Это замусоривает файл и мешает анализу структуры сессии.
