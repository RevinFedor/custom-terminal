...
### Solution
1. **Dynamic Resolution:** Система сначала проверяет `~/.gemini/projects.json` для поиска slug-а проекта, иначе откатывается к SHA256 хешу. Это критично для новых проектов, которые только что были проиндексированы Gemini CLI.
2. **Long-lived Watcher:** Увеличен таймаут вочера до 5 минут. Вочер привязан к жизненному циклу вкладки.
