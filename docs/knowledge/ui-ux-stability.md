...
## 15. InfoPanel Layout: Scroll vs Editor Height Conflict

### Проблема
InfoPanel растягивала весь интерфейс терминала по вертикали при большом объеме заметок. Скролл на всей панели делал невозможным использование Markdown-редактора (CodeMirror), так как он не получал фиксированной высоты и рос бесконечно.

### Решение: Dual-Zone Layout
InfoPanel разделена на два независимых контейнера через Flexbox:
1.  **Верхняя зона (Sections):** Контейнер с `overflow-y-auto min-h-0 shrink`. Содержит AI Session, Commands и т.д. Сжимается и скроллится сам, если места мало.
2.  **Нижняя зона (Tab Notes):** Контейнер с `flex-1 min-h-0 overflow-hidden`. Это "запирает" высоту редактора. CodeMirror видит фиксированный размер контейнера и включает свой внутренний скроллбар.
