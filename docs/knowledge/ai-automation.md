...
2. **Определение головы:** Берем самую последнюю запись в последнем файле цепи.
3. **Обход:** Идем от "головы" назад к корню, используя поле `parentUuid`.
4. **Компакция:** Если встречаем маркер компакции, используем логику **Compact Gap Recovery** (см. ниже).
5. **Bridge Following:** Если `parentUuid` отсутствует (начало файла), ищем запись-мост (`_isBridge`) в других файлах цепи.
6. **Результат:** Инвертируем собранный список.
