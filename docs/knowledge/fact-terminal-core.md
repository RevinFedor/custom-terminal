...
- **Interceptive Blocking:** На текущем этапе (Phase 1) блокировка очереди выполняется через перехват `terminal:input` в Main-процессе. Это позволяет мгновенно остановить впрыск ответа AI, как только пользователь нажал первую клавишу, не дожидаясь цикла рендеринга и парсинга буфера xterm.
- **Цель:** Найти стабильные якоря (anchor sequences), обозначающие начало и конец редактируемой области.
- **Многострочность:** Особое внимание уделяется детекции расширения инпута при вводе длинных промптов, чтобы отличить пустую строку от начала многострочного ввода.
