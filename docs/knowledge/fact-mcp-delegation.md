...
1. **`delegate_to_claude(prompt)`**: Создает новую вкладку и запускает Claude. Возвращает `taskId`.
2. **`continue_claude(taskId, prompt)`**: Отправляет сообщение в существующую сессию. Если приложение было перезапущено, автоматически выполняет `claude --resume` в том же табе.
3. **`list_sub_agents()`**: Возвращает Gemini список всех его суб-агентов (включая тех, что были созданы до перезапуска приложения).
...
3. При переходе Claude в состояние IDLE, результат извлекается из логов и впрыскивается в Gemini PTY через `safePasteAndSubmit` в специальных тегах `[Claude Sub-Agent Response]`.
