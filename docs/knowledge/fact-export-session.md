...
4. **Full Export:** При вызове без указания диапазона (через кнопку Copy Session) экспортирует все сообщения в формате `Human: ... / Assistant: ...`.

---

## 7. Интеграция с Update Docs
Пайплайн автоматического обновления документации (`handleUpdateDocs`) теперь является универсальным:
1. **Детекция сессии:** Система считывает `claudeSessionId` или `geminiSessionId` из активной вкладки.
2. **Экспорт контекста:** Для Gemini используется тот же механизм `gemini:copy-range`, что и для обычного копирования.
3. **Анализ:** Полученный Markdown передается в Gemini для выделения изменений (Signal vs Noise), независимо от того, какой агент (Claude или Gemini) использовался во время разработки.
