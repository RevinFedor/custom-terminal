...
- **Динамическая статусаная строка (UX):** Вместо статичного "Processing..." во время работы отображается анимированный синий спиннер и текст текущего этапа (Экспорт сессии... → API gemini ~45Kt... → Post-check... → Открытие Haiku...). Это обеспечивает прозрачность процесса и не меняет высоту блока ActionsPanel.
- **Post-check (Автоматическая верификация):** Если тогл в настройках включен, система выполняет дополнительный легковесный запрос после основного анализа.
    - **Цель:** Поиск "шрамов формата данных" (Data Scars) и предотвращение засорения `CLAUDE.md`.
    - **Механика:** Результат верификации дописывается в конец основного ответа перед созданием сессии Haiku.
