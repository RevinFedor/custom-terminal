...
### Значимость для AI-to-AI
Инструменты анализа буфера терминала должны проверять состояние `cell.isInverse()`. Если флаг активен, визуальный "фон" на самом деле хранится в атрибуте Foreground цвета ячейки. Это объясняет, почему попытки найти границы сообщений по явным кодам Background RGB провалились.
