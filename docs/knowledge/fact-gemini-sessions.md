...
- **Short ID Support:** Пользователь может вставить как полный UUID, так и короткий 8-символьный ID из имени файла.
- **Resolution:** Если введен 8-символьный hex, система сканирует папку чатов текущего проекта, находит файл с подходящим префиксом и автоматически "разворачивает" его в полный UUID перед сохранением в store.
- **Real-time Feedback:** Инпут в боковой панели валидирует ID на лету (см. `knowledge/fact-session-capture.md`).
