...
- **Main Process:** Управляет `node-pty` (терминальными сессиями), SQLite базой данных (таблицы `projects`, `tabs`, `tab_history`, `favorites`), файловой системой и жизненным циклом внешних процессов.
    - **КРИТИЧЕСКОЕ ПРАВИЛО:** Запрещено использование `execSync`. Все системные вызовы (pgrep, lsof, ps) должны быть асинхронными (через `execAsync`). См. `knowledge/ui-ux-stability.md`.
...
- **Focus & Activity Model:** 
    - **Area Focus:** Введено понятие `activeArea` ('projects' | 'workspace'). Переключение через `onMouseDown`. См. `knowledge/ui-ux-stability.md`.
...
- **Persistent Workspace:** Компонент `Workspace` остается смонтированным всегда. Переход между Dashboard и Workspace через `visibility: hidden`. Это сохраняет контексты терминалов. См. `knowledge/ui-ux-stability.md`.
