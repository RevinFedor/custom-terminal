...
- **AI Integrations:**
    ...
    - **Timeline & Export Engine:** Асинхронный парсинг JSONL файлов с использованием алгоритма **Backtrace** для фильтрации отменённых (Undo) веток диалога. 
        ...
        - **Unified Pipeline:** Все четыре механизма (`claude:get-timeline`, `claude:copy-range`, Copy Session, Update Docs) используют идентичный пайплайн: `resolveSessionChain` (загрузка файлов цепи) → генерация единой `merged recordMap` → алгоритм **Backtrace** с применением `compact recovery` и защитой от циклов в мостах. **КРИТИЧЕСКОЕ ПРАВИЛО:** Любой новый IPC-хендлер, который работает с UUID из timeline, ОБЯЗАН использовать `resolveSessionChain`, а не загружать единичный JSONL файл. Иначе UUID из родительских сессий (до Plan Mode) не будут найдены.
        - **Update Docs Flow:** Фича "Update Docs" переведена на Unified Pipeline. Вместо `/export` в терминале данные берутся напрямую из сессии, сохраняются в `.txt` файл, а в чат Gemini отправляется составной промпт: `Settings Prompt + File Path + Manual Input`. См. `features/docs-update.md`.
...
- **Large Input (Two-Tier Paste):** Из-за ограничения macOS TTYHOG (1024 bytes) любые данные > 1024 байт должны быть разбиты на чанки. Реализованы два механизма: (1) **Simple chunking** в `terminal:input` для user paste (Ctrl+V) — чанки < 900 байт с 10ms задержкой, без sync markers; (2) **`safePasteAndSubmit`** для программных операций (Handshake, send-command, Rewind) — чанки с sync marker verification, необходимой потому что код шлёт Enter (`\r`) сразу после текста. См. `knowledge/terminal-core.md`.
