...
- **Large Input (safePasteAndSubmit):** Для вставки промптов > 1024 байт (включая Rewind и Handshake) используется чанковая запись с подтверждением отрисовки каждого кадра. Это необходимо из-за ограничения macOS TTYHOG (1024 bytes) и особенностей Ink TUI. См. `knowledge/terminal-core.md`.
