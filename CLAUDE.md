...
MCP-инструмент `docs_search` доступен для поиска по базе знаний. **Используй проактивно:**
- Перед изменениями в сложных подсистемах (JSONL, PTY, Timeline, MCP delegation, Zustand store)
- При отладке незнакомых багов — ищи по симптомам
- Когда нужен архитектурный контекст или известные workarounds

Запрос должен быть на английском, конкретные технические термины. Пример: `"zustand state mutation stale update"`, `"paste hang timeout sync marker"`.
...
- **Не используй polling и таймауты для ожидания состояний:** Проект event-driven — для каждого состояния уже есть сигнал (OSC 133, `geminiSpinnerBusy` → `processGeminiQueue`, `geminiResponseQueue`, IPC events). Перед написанием `setTimeout`/`setInterval` для ожидания — найди существующий event-driven механизм в кодовой базе. `setTimeout` допустим только для debounce, не для "подождать пока X будет готов".
...
- **Orchestration Timeline:** [`fact-timeline.md`](docs/knowledge/fact-timeline.md) — визуализация сложных цепочек агентов (Tree View, группировка раундов, абсолютное наложение, ширина 32px).
