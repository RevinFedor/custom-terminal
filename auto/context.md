...
### 6. Golden Session Pattern (Оптимизация)
Для тестов Gemini (особенно Timeline и Rewind) используется стратегия клонирования:
- **Проблема:** Наполнение истории 15-ю сообщениями через PTY занимает ~50 секунд и часто сбоит из-за лагов AI-ответов.
- **Решение:** Тест ищет файл `session-golden.json` в папке чатов. Если его нет — создает. Если есть — вызывает `gemini-f` (True Fork).
- **Эффект:** Тест стартует с уже заполненным таймлайном за 2 секунды. Это стандарт для тестирования сложного UI, не зависящего от свежести ответов модели.
