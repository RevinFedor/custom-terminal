...
function parseSubAgentMeta(content) {
  ...
  const isResponse = (content.startsWith('[Claude Sub-Agent Response]') ||
                     ...
                     content.includes('[/Claude Sub-Agent Response]');
  ...
  if (isResponse) {
    ...
    if (footerMatch) {
      ...
      const modelMatch = footer.match(/Model:\s*(.+?)(?:\s*\||$)/);
      if (tabMatch) subAgentName = tabMatch[1].trim();
      if (taskMatch) subAgentTaskId = taskMatch[1].trim();
      if (modelMatch) subAgentModel = modelMatch[1].trim();
    } else {
      console.log('[SubAgentMeta] No footer found in response (' + content.length + ' chars), preview: ' + content.slice(0, 80).replace(/\n/g, '\\n'));
    }
  }
  ...
  return {
    ...
    subAgentName: subAgentName || (subAgentModel ? `Claude (${subAgentModel})` : undefined),
...
function register({ projectManager, geminiUtils, terminals, terminalProjects, geminiHistoryWatchers }) {
  ...
  ipcMain.handle('gemini:get-timeline', async (event, { sessionId, cwd }) => {
    ...
    try {
      ...
      const subAgentCount = entries.filter(e => e.isSubAgent).length;
      console.log('[Gemini Timeline] Returning', entries.length, 'entries' + (subAgentCount > 0 ? ' (' + subAgentCount + ' sub-agent)' : ''));
      return {
        success: true,
        entries,
        sessionBoundaries: [],
